...
arquivo_ontem_csv = 'chegada_' + ontem.strftime('%Y-%m-%d') + '.pdf'
arquivo_amanha_csv = 'chegada_' + amanha.strftime('%Y-%m-%d') + '.pdf'

nome_arquivo_csv = 'chegada_' + datetime.now(fuso_horario).strftime('%Y-%m-%d') + '.pdf'
arquivo_csv = st.file_uploader("Carregar PDF de Chegadas (" + nome_arquivo_csv + ")", type=['pdf'])
...
if (arquivo_csv) and ((nome_arquivo_csv == arquivo_csv.name) or (arquivo_ontem_csv == arquivo_csv.name) or (arquivo_amanha_csv == arquivo_csv.name)):
    ...
    df_csv.columns = ['veiculo', 'nulo1', 'horario','nulo2', 'motorista', 'nulo3', 'nulo4','nulo5', 'linha'][:len(df_csv.columns)]
    ...
    df_csv = df_csv.drop(df_csv.columns[[1, 3, 5, 6, 7]], axis=1)
    ...
    df_csv[segunda_coluna] = np.where((df_csv[segunda_coluna] < '03:00'), df_csv[segunda_coluna] + delta + timedelta(days=1), df_csv[segunda_coluna] + delta)
